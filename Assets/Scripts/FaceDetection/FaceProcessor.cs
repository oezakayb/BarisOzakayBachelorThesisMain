...
// Taken from OpenCvSharp Demo package and altered
        ...
        where T: UnityEngine.Texture
    {
        ...
        /// </summary>
        public Point[] MarkDetected(bool drawSubItems = false)
        {
            // mark each found eye
            foreach (DetectedFace face in Faces)
            {
                //eyeDistance = (float) faceOutline[39].DistanceTo(faceOutline[42]);
                
                // face rect
                //Cv2.Rectangle((InputOutputArray)Image, face.Region, Scalar.FromRgb(255, 0, 0), 2);

                // Get the points 1 to 17 (jawline)
                List<Point> jawlinePoints = face.Marks.Take(17).ToList();

                Point center;
                
                try
                {
                    center = new Point(jawlinePoints[0].X + Mathf.Abs(jawlinePoints[0].X - jawlinePoints[16].X) / 2,
                        jawlinePoints[0].Y);
                }
                catch (Exception e)
                {
                ...
                List<Point> semiEllipsePoints = GenerateSemiEllipse(
                    center.X - jawlinePoints[0].X,
                    center.Y - face.Region.Top,
                    center.X,
                    center.Y,
                    20
                    );
                
                List<Point> faceShapePoints = new List<Point>(jawlinePoints);
                faceShapePoints.AddRange(semiEllipsePoints);

                Scalar color = Scalar.FromRgb(255, 0, 0);
                
                double deltaY = jawlinePoints[16].Y - jawlinePoints[0].Y;
                double deltaX = jawlinePoints[16].X - jawlinePoints[0].X;
                double tilt = Mathf.Atan2((float)deltaY, (float)deltaX) * Mathf.Rad2Deg;
                
                double distanceLeft = jawlinePoints[0].X - face.Region.Left;
                double distanceRight = face.Region.Right - jawlinePoints[16].X;

                if (Mathf.Abs((float) tilt) < 7 && Mathf.Abs((float) (distanceLeft - distanceRight)) < 20)
                {
                    color = Scalar.FromRgb(0, 255, 0);
                }

                Point[] convexHull = Cv2.ConvexHull(faceShapePoints);
                
                Cv2.Polylines(Image, new IEnumerable<Point>[]{convexHull}, true, color, 2);

                if (color == Scalar.FromRgb(0, 255, 0))
                {
                    return convexHull;
        ...
        /// <summary>
        /// Generates points for a semi-ellipse connecting three key points.
        /// </summary>
        private List<Point> GenerateSemiEllipse(double a, double b, double centerX, double centerY, int numPoints)
        {
            List<Point> ellipsePoints = new List<Point>();
            
            for (int i = 0; i <= numPoints; i++)
            {
                double angle = Mathf.Lerp(0, Mathf.PI, (float)i / numPoints); // Angle from 0 to PI
                double x = centerX + a * Mathf.Cos((float)angle);
                double y = centerY - b * Mathf.Sin((float)angle);
